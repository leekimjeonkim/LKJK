...
if len(sys.argv) < 2:
    print("테스트: %s <filename> [samplerate]" % sys.argv[0])
    sys.exit(1)

filename = sys.argv[1]

downsample = 1
samplerate = 44100 // downsample #비트레이트 44100 고정.
if len( sys.argv ) > 2: samplerate = int(sys.argv[2])

win_s = 4096 // downsample # fft size #fast furier tramsform 사이즈, down 샘플 값을 조정해야함
hop_s = 512  // downsample # hop size

s = source(filename, samplerate, hop_s)
samplerate = s.samplerate

tolerance = 0.8 #똘레랑스 똘레랑스!!!!!
...
#bpm 부에서 설정한 프레임값과 동일, 컴파일 순서 중요시됨.
total_frames = 0
while True:
    samples, read = s()
...
times = [t * hop_s for t in range(len(pitches))] #시간은 song tempo 부분과 동기화.
